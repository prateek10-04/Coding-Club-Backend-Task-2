...
class akshay:
    ...
    def display(self):
        print("The number to the left of each item displays its ID number\n")
        c2=int(input('Enter \n1 for Electronics\n2 for Food items\n3 for Stationery\n4 for Toiletries\n5 for Plastics\n6 if you want to search by name\n'))
        n=1
        if c2==1:
            ...
            for names in item_list_name_id.keys():
                temp_name_spilt=names.split(' ')

                for i in range (0,len(temp_name_spilt)):
                    if search.casefold()==(temp_name_spilt[i].casefold()):
                        print(str(item_list_name_id.get(names))+" "+names+" Rs "+str(item_list_id_price.get(item_list_name_id.get(names))))
        else:
            print("Wrong input!Please try again")
            exit()

    def addtocart(self):
        add=int(input(print("Enter\n1 if you want to add by ID number\n2 if you want to add by item name")))
        if add==1:
    ...
    def delete_item(self):
        del_item=int(input("Enter\n1 if you want to delete by ID\n2 if you want to delete by name\n"))
        if del_item==1:
            ...
            del_name=input("Enter name of product to be deleted\n")
            self.takenitem.pop(item_list_name_id.get(del_name))
            self.item_name.pop(del_name)
    ...
    def displaybill(self,id4,names,bhavan,roomno):
        ...
        if (1000<=total<=2000):
            ...
            print(f"Congratulations you saved Rs {total2-total}")
...
if login.casefold()=='Student'.casefold():
    ...
    while(True):
        ...
        try:
            ...
            print("Wrong input!Please try again")
            continue
elif login.casefold()=='Admin'.casefold():
